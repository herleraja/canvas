...
export default class FlowsProperties extends React.Component {
	...
	getPropertyDefName(node) {
		if (node.op) {
			console.log("🚀 ~ FlowsProperties ~ getPropertyDefName ~ node:", node);
			const foundName = this.availableParamDefs.find((name) => name.startsWith(node.op));
			console.log("🚀 ~ FlowsProperties ~ getPropertyDefName ~ availableParamDefs:", this.availableParamDefs);
			console.log("🚀 ~ FlowsProperties ~ getPropertyDefName ~ foundName:", foundName);
			if (foundName) {
				return {
					fileName: foundName,
					type: PARAMETER_DEFS
